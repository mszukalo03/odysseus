...
> A tool JUST added to `FUNCTION_TOOL_SCHEMAS` will not actually reach the model in most turns and will be denylisted in Plan Mode — the pipeline has ~7 registration points, discovered while wiring `get_home_weather`/`query_ithaca_tile` (2026-07-25, see `src/tools/ithaca.py` for a worked example of a simple read-only tool).
