...
| `GET /api/ithaca/digest` | Session / `ithaca:read` | Latest daily-digest md from the Obsidian Local REST API, split by headings; Software Updates table parsed to rows + enriched with per-app links (5-min TTL cache) |
