...
- [x] ~~**MCP server** — expose feed read/search/subscribe to the AI agent~~ — done differently than originally scoped: rather than an MCP server, four native function-calling tools were added (`list_rss_feeds`/`get_rss_articles`/`summarize_rss_articles`/`mark_rss_article`, `src/tools/feed.py`), matching how Ithaca exposes its own domain. See [[features]] and [[systems]]. *(done 2026-08-08)*
