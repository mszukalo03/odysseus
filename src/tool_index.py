...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "get_ithaca_weather": "Get the user's LOCAL home weather: current conditions plus the upcoming hourly forecast, queried directly from OpenWeatherMap for the user's configured coordinates (same source as the Ithaca hub's Weather tile). Use for 'what's the weather', 'weather forecast', 'is it going to rain', 'how hot/cold is it' about the user's own location — prefer this over web_search for that.",
    "get_ithaca_software_updates": "List pending updates for the user's self-hosted homelab apps (Radarr, Sonarr, Prowlarr, Transmission, Jellyfin, Jellyseerr/Seerr, and any Flatpak apps tracked by the automation) — the same data as the Ithaca hub's Software Updates tile. Reads the latest weekly digest markdown written by the n8n automation. Returns per-app current vs available version, whether an update is available, and a short changelog summary. Use for 'what needs updating', 'any app updates', 'is X up to date', 'homelab updates'.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        # Local/home weather — routed to the Ithaca hub's direct OpenWeatherMap
        # query, not a web search. web_search/web_fetch stay available too (via
        # the "current"/"news"/"latest" hint above) as a fallback for weather
        # elsewhere ("weather in Tokyo") or if the tool isn't configured.
        frozenset({"weather", "forecast", "rain", "raining", "snow", "snowing",
                   "temperature outside", "how hot", "how cold"}):
            {"get_ithaca_weather", "web_search"},
        frozenset({"software update", "software updates", "app update", "app updates",
                   "updates available", "need updating", "needs updating",
                   "up to date", "outdated", "homelab", "radarr", "sonarr",
                   "prowlarr", "jellyfin", "jellyseerr", "transmission", "flatpak"}):
            {"get_ithaca_software_updates"},
