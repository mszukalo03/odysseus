...
_register(
    ...
    ToolEffect.BROKERED_NETWORK_READ,
    # Weather comes from OpenWeatherMap; a tile's data comes from whatever
    # database the user connected it to — neither is server-authored text.
    result_integrity=ResultIntegrity.EXTERNAL_UNTRUSTED,
)
_register(
    {"list_rss_feeds", "get_rss_articles"},
    ToolEffect.READ_PRIVATE,
    # Feed/article titles are authored by whatever remote site published
    # them, not the user or this server.
    result_integrity=ResultIntegrity.EXTERNAL_UNTRUSTED,
)
_register(
