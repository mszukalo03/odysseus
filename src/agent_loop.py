...
`calendar` accepts a name ("Main") or short-id prefix.""",
    "get_ithaca_weather": "- ```get_ithaca_weather``` — Get the user's LOCAL home weather (current conditions + upcoming hourly forecast), queried directly from OpenWeatherMap — the same data as the Ithaca hub's Weather tile. No arguments needed; optionally `{\"refresh\": true}` to bypass the cache. Prefer this over web_search for the user's own weather.",
    "get_ithaca_software_updates": "- ```get_ithaca_software_updates``` — List pending updates for the user's self-hosted apps (Radarr, Sonarr, Prowlarr, Jellyfin, etc.), from the latest n8n-generated digest — the same data as the Ithaca hub's Software Updates tile. No arguments needed; optionally `{\"refresh\": true}` to bypass the cache.",
